		...
		for r in range(MAX_ROOMS):
			...
			if not failed:
				...
				if num_rooms == 0:
					...
					self.player.worldPosition = [center[0]*S, center[1]*S, (PLAYER_HEIGHT/2)+0.05]
		...
		for y in range(self.height):
			for x in range(self.width):
				...
				if self.map[x][y].prop:
					p = self.map[x][y].prop
					self.own.worldPosition = [(x*S)+rand(-1,1),(y*S)+rand(-1,1), 2]
					if p == 'LifePotion':
						self.own.worldPosition.z = 0
					prop = logic.getCurrentScene().addObject(p, self.own)
